...
Simple scripts to set up a fresh Raspbian or Ubuntu Server install with TAK Server. The server will generate certs if you want to use TLS. The script `./mk-client-cert.sh` can be used to generate a client certificate. `reload-certs.sh` will restart the TAK Server and reload certs.
